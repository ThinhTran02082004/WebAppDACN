...
// Middleware
app.use(cors({
  origin: process.env.FRONTEND_URL || ['http://localhost:3000', 'http://localhost', 'http://10.0.130.25:3000'],
  credentials: true,
...
const startServer = async () => {
  try {
    ...
    // Bind explicitly to 0.0.0.0 so server is reachable from other devices on the LAN
    const BIND_HOST = process.env.BIND_HOST || '0.0.0.0';
    server.listen(PORT, BIND_HOST, () => { // Use server instead of app
