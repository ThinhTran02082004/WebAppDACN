...
export const ensureLivekitGlobals = () => {
  ...
  try {
    ...
    // Try to register globals, but ignore error if views are already registered
    try {
      registerGlobals();
      globalsRegistered = true;
      console.log('[livekit] WebRTC globals registered successfully');
    } catch (registerError: any) {
      // If error is about duplicate view registration, it's OK - views are already registered
      if (registerError?.message?.includes('same name') || registerError?.message?.includes('RTCVideoView')) {
        console.log('[livekit] WebRTC views already registered, skipping');
        globalsRegistered = true;
      } else {
        throw registerError;
      }
