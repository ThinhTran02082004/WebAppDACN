...
// @ts-ignore - @livekit/react-native exports may not have proper type definitions
import { Room, Participant, Track, VideoTrack } from '@livekit/react-native';
import {
  Permission,
  PERMISSIONS,
  RESULTS,
...
const VideoCallScreen = () => {
  ...
  const updateVideoTiles = useCallback(() => {
    ...
    const pushTrack = (track: any, participant: any, trackSid?: string, trackName?: string) => {
      ...
      tiles.push({
        id: trackSid ?? `${participant.sid}-${trackName ?? 'video'}`,
        streamUrl,
        participantName:
          participant.name ||
    ...
    // Local participant video tracks - iterate through videoTracks Map
    if (room.localParticipant.videoTracks) {
      room.localParticipant.videoTracks.forEach((track: any, trackSid: string) => {
    ...
    room.remoteParticipants.forEach((participant: any) => {
      if (participant.videoTracks) {
        participant.videoTracks.forEach((track: any, trackSid: string) => {
  ...
  const connectToRoom = useCallback(async () => {
    ...
    // Create Room - in v2.9.5, Room constructor should work after ensureLivekitGlobals()
    // The globals registration sets up the necessary polyfills for Room to work
    const room = new Room({
      adaptiveStream: true,
      dynacast: true,
    ...
    try {
      ...
      // Connect using the URL - LiveKit SDK will automatically add /rtc path
      // Do NOT manually add /rtc or /ws - let LiveKit SDK handle it
      await room.connect(effectiveWebSocketUrl, token, { 
        autoSubscribe: true
      });
