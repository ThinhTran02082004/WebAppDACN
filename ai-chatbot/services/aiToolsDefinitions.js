...
const tools = {
    functionDeclarations: [
        {
            name: "triageSpecialty",
            description: "Phân loại triệu chứng tiếng Việt để đề xuất chuyên khoa phù hợp. BẠN PHẢI gọi tool này NGAY LẬP TỨC khi người dùng mô tả triệu chứng hoặc bệnh lý (ví dụ: 'đau ngực trái', 'khó thở khi gắng sức', 'sốt cao', 'đau bụng'). Tool này sẽ tự động phân tích và đề xuất chuyên khoa phù hợp nhất, đồng thời đánh giá mức độ nguy hiểm (normal/urgent/emergency). Sau khi nhận kết quả, bạn PHẢI lưu department vào conversation state và trả lời cho người dùng bằng tiếng Việt tự nhiên, giải thích vì sao nên khám khoa đó.",
            parameters: {
                type: "OBJECT",
                properties: {
                    symptomsText: {
                        type: "STRING",
                        description: "Mô tả triệu chứng đầy đủ do bệnh nhân cung cấp (ví dụ: 'đau ngực trái kèm khó thở khi gắng sức', 'sốt cao 39 độ kèm đau đầu dữ dội')."
                    },
                    age: { 
                        type: "NUMBER", 
                        description: "Tuổi của bệnh nhân (tùy chọn, để điều chỉnh đề xuất chuyên khoa)",
                        nullable: true
                    },
                    gender: { 
                        type: "STRING", 
                        enum: ["male", "female", "other"],
                        description: "Giới tính của bệnh nhân (tùy chọn)",
                        nullable: true
                    }
                },
        ...
        {
            name: "getDoctorInfo",
            description: "Lấy thông tin chi tiết của một bác sĩ (giới thiệu, chuyên khoa, bệnh viện, học vấn, kinh nghiệm). Gọi tool này khi người dùng hỏi thông tin/giới thiệu về một bác sĩ cụ thể.",
            parameters: {
                type: "OBJECT",
                properties: {
                    name: { type: "STRING", description: "Tên bác sĩ cần tra cứu (ví dụ: 'Nguyễn Quốc Duy')." },
                    doctorId: { type: "STRING", description: "ID của bác sĩ (tùy chọn, nếu đã có ID)." }
                },
                required: ["name"]
            }
        },
        ...
        {
            name: "searchDrugs",
            description: "Tra cứu thông tin thuốc & hoạt chất tương ứng với triệu chứng hoặc bệnh. Tool này sử dụng AI để tìm kiếm thông tin y khoa về thuốc phù hợp. BẠN PHẢI gọi tool này khi người dùng hỏi về thuốc, muốn tư vấn thuốc, hoặc hỏi 'có thuốc nào không' (ví dụ: 'Tôi bị đau bụng, có thuốc nào không?', 'Sốt cao uống thuốc gì?', 'Thuốc nào trị ho?'). Sau khi nhận kết quả, bạn có thể sử dụng thông tin này để tư vấn hoặc gọi checkInventoryAndPrescribe để kiểm tra kho thuốc.",
            parameters: {
                type: "OBJECT",
                properties: {
                    query: {
                        type: "STRING",
                        description: "Mô tả triệu chứng hoặc tên bệnh để tra thuốc (ví dụ: 'đau đầu', 'sốt cao', 'ho nhiều', 'đau dạ dày')."
                    }
                },
                required: ["query"]
            }
